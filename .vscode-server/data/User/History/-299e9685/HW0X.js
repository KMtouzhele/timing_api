...
const getRaces = async (req, res) => {
    try {
        ...
        return res.status(404).json({ code: 500, result: 'Race not found' });
    }
};
...
const getLeaderboards = async (req, res) => {
    ...
    for (let entrantIndex = 0; entrantIndex < race.entrants.length; entrantIndex++) {
        ...
        let totalTime = 0;
        let completedLaps = 0;
        const position = startingPositions[entrantIndex];
        for (const lap of laps) {
            const lapData = lap.lapTimes.find(lt => lt.entrant === position && lt.crashed === false);
            if (lapData) {
