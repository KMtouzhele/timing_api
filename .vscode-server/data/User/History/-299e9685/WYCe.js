...
const qualify = (req, res) => {
    ...
    for (const entrant of entrants) {
        ...
        driverSkills.push({ uri: entrant, skill: skill });
    }
        ...
        const driverSkills = await Promise.all(entrants.map(async (entrant) => {
            try {
                ...
                if (carResponse.data && carResponse.data.code === 200) {
                    ...
                    if (driverResponse.data && driverResponse.data.code === 200) {
                        const driverData = driverResponse.data;
                        const raceSkill = driverData.result.skill.race;
                        const streetSkill = driverData.result.skill.street;
...
const simulateLaps = async (req, res) => {
    ...
    for (const entrant of entrants) {
        try {
            ...
            const random = parseFloat(lapResponse.data.result.randomness);
