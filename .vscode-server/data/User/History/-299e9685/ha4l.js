...
const simulateLaps = async (req, res) => {
    ...
    const { trackType, baseLapTime } = track;  // Extract from track data
    const totalLaps = track.laps;
    let laps = [];

    while (count < totalLaps) {
        let entrantsLap = [];

        // Use for...of to handle async requests properly
        for (const entrant of entrants) {
            try {
                const getLapURL = entrant + '/lap';

                // Send trackType and baseLapTime in the request body
                const lapResponse = await axios.post(getLapURL, {
                    trackType,
                    baseLapTime
                }, { httpsAgent });

                if (lapResponse.data.code !== 200) {
                    return res.status(500).json({ code: 500, result: "Error generating lap" });
                }
    ...
    console.log(laps);
    // Return the laps data as the response
