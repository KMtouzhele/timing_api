...
const updateMovie = (req, res) => {
    ...
    // Static values for the updated movie
    const newName = 'Kill Bill';
    const newYear = '2003';
    const newDirector = 'Quentin Tarantino';

    // Prepare the SQL update statement
    const sql = 'UPDATE movie SET name = ?, year = ?, director = ? WHERE id = ?';

    // Execute the query with static values
    dbconn.query(sql, [newName, newYear, newDirector, id], (err, result) => {
        if (err) {
            console.error(err); // Log the error for debugging
            return res.status(500).send('Internal Server Error');
        }
        ...
        res.status(200).json({ message: 'Movie updated successfully', id, name: newName, year: newYear, director: newDirector });
    });
};
