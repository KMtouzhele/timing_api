...
const getRacesForTrack = (req, res) => {
    ...
    dbconn.query(
        'SELECT races.id, races.track, races.entrants, races.startingPositions, races.laps, tracks.name, tracks.type FROM races JOIN tracks on races.track = tracks.id where races.track = ?',
        [trackId],
        (err, rows) => {
            if (err) {
                return res.status(500).json({
            ...
            const races = rows.map((race) => {
                return {
                    ...
                    track: {
                        name: race.name,
                        uri: `https://lab-2105cf46-fd70-4e4b-8ece-4494323c5240.australiaeast.cloudapp.azure.com:7042/tracks/${race.track}`
                    },
                    entrants: JSON.parse(race.entrants),
                    startingPositions: JSON.parse(race.startingPositions),
